...
const upload = multer({
  storage: multerS3({
    ...
    // Instead of static 'contentDisposition', provide a function to set dynamically:
    contentDisposition: (req, file, cb) => {
      const isVideo = /\.(mp4|mov|avi|mkv|webm)$/i.test(file.originalname);
      if (isVideo) {
        cb(null, 'inline'); // Allow inline playback
      } else {
        cb(null, 'attachment'); // Other files default to download
      }
    },
