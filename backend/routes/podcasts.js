  ...
  try {
    ...
    if (isVideo) {
      try {
        ...
        const tempPath = path.join('/tmp', `temp-${Date.now()}.mp4`) || path.join('uploads', `temp-${Date.now()}.mp4`); 
...
router.post('/:id/play', async (req, res) => {
  try {
    ...
    // ✅ Return playable URL and info
    res.json({
      success: true,
      message: 'Podcast play started',
      title: podcast.title,
      plays: podcast.plays,
      mediaType: podcast.mediaType,
      url: podcast.s3Url,           // 🎯 main playable link
      thumbnail: podcast.thumbnailUrl || null,
